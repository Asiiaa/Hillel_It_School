...
function students(student){
    ...
    let gpa = (student) => {
        let newArray = structuredClone(student); //создаем новый массив с элементами старого
        let sum;
        for (let i = 0; i < student.length; i++) {
            sum = student[i].marks.reduce(getSum); //сумма оценок
            function getSum(a, b) {
                return a + Math.round(b); //среднее значение
            };
            newArray[i].meanValue = sum/(student[i].marks.length); //добавляем среднее значение ко всем объектам
        }
        return newArray;
    };
    console.log(gpa(student));
    
    let remandStudent = (gpa) => {
        let newArray = [];
        for (let i = 0; i < gpa.length; i++) {
            if (gpa[i].meanValue < 60) {
                newArray.push(gpa[i].name); // список имен студентов на отчисление
            };
        };
        return newArray;
    };
    console.table(remandStudent(gpa(student)));

    let median = (student) => {
        let newArray = structuredClone(student); //создаем новый массив с элементами старого
        for (let i = 0; i < student.length; i++) {
            newArray[i].marks.sort((a, b) => a - b);
            if (newArray[i].marks.length % 2) {
                newArray[i].median = newArray[i].marks[Math.floor(newArray[i].marks.length / 2)];
            } else {
                newArray[i].median = (marks[marks.length / 2] + marks[marks.length / 2 - 1]) / 2;
            };
        };
        return newArray;
    };

    console.log(median(student))
    ...
    let getRandomIntInclusive = (min, max) => {
        ...
        for (let i = Math.floor(Math.random() * (max - min + 1)) + min; i < newArray.length; i++) { //начальное значение = рандомному числу
        newArray[i] === Math.floor(Math.random() * (max - min + 1)) + min; //сравниваю индексы элементов и рандомное число
        newArray.push(newArray[i]); //добавляю тот элемент индекс которого совпал
        return newArray;
    }
    }
    ...
    let list = (student) => {
        let newArray1 = structuredClone(gpa(student));
        let newArray = [newArray1.sort((a, b) => a.meanValue < b.meanValue ? 1 : -1)]; //сравниваем среднее значение

        for (let i = 0; i < newArray1.length; i++) {
            newArray.push(`${newArray1[i].name} - ${newArray1[i].meanValue}`);//перепробовала все способы, получилось вывести только строками
        }
        newArray.shift(0);// под нулевым индексом показывает весь массив, не смогла понять в чем ошибка или это побочный эффект ссылок, пришлось его отдельно удалять
        return newArray;
    }
    console.table(list(student));

    let list5 = (list) => {
        let newArray1 = structuredClone(list(student));
        let newArray2 = [];
        for (let i = 0; i < 5; i++) {
